...
int main(int argc,char **argv){
    ...
    // 初始化读写线程通信所用的信号量
    sem_init(&rwsem,0,0);
    // 连接服务器成功，启动接收线程负责接受数据
    std::thread readTask(readTaskHandler,clientfd); // pthread_create
    readTask.detach();  // pthread_detach
 

    // main线程负责接受用户输入，负责发数据
    for(;;)
    {
        // 显示页面菜单，登录，注册，退出
        cout<<"==================="<<endl;
...
void showCurrentUserData(){
    ...
    if(!g_currentUserFriendList.empty())
    {
        for(Group &group:g_currentUserGroupList){
            cout<<"群组id:"<<group.getId()<<"  群组名："<<group.getName()<<"  群组描述"<<group.getDesc()<<endl;
    ...
    if(responsejs.contains("offlinemsg"))
    {
        ...
        for(string &str:vec)
        {
            ...
            if(ONE_CHAT_MSG==msgtype)
            {
                ...
                cout<<"离线群消息["<<js["groupid"]<<"]"<<js["time"].get<string>()<<"["<<js["id"]<<"]"<<js["name"].get<string>()
                    <<"  said:"<<js["msg"].get<string>()<<endl;
                cout<<"==========================================="<<endl;           
            }
        }
...
void mainMenu(int clienfd){
    ...
    while(isMainMenuRunning){
        ...
        it->second(clienfd,commandbuf.substr(idx+1,commandbuf.size()-idx));// 调用处理函数
    }
}
